...
# Sauvegarde de l'ancien fichier de configuration des dépôts
if [ ! -f /etc/opkg/distfeeds.conf.bak ]; then
    cp /etc/opkg/distfeeds.conf /etc/opkg/distfeeds.conf.bak
fi

# Écriture des nouvelles sources dans le fichier des dépôts
cat << EOF > /etc/opkg/distfeeds.conf
src/gz wisgateos_core https://downloads.openwrt.org/releases/21.02.1/targets/mipsel_24kc/generic/packages
src/gz wisgateos_base https://downloads.openwrt.org/releases/21.02.1/packages/mipsel_24kc/base
src/gz wisgateos_lora_open https://downloads.openwrt.org/releases/21.02.1/packages/mipsel_24kc/lora_open
src/gz wisgateos_openwrt_updates https://downloads.openwrt.org/releases/21.02.1/packages/mipsel_24kc/openwrt_updates
src/gz wisgateos_packages https://downloads.openwrt.org/releases/21.02.1/packages/mipsel_24kc/packages
src/gz wisgateos_rak https://downloads.openwrt.org/releases/21.02.1/packages/mipsel_24kc/rak
EOF
...
opkg install python3 unzip python3-paho-mqtt || echo "Certains paquets sont déjà installés."
